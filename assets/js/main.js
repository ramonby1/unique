...
const I18N = {
  en:{
    ...
    'section.highlights':'This week’s highlights','section.explore':'Explore','footer.demo':'Demo',
  ...
  es:{
    ...
    'section.highlights':'Destacados de la semana','section.explore':'Explora','footer.demo':'Demostración',
    ...
    themes:['Deportes','Aventura','Gastro','Tour','Recreativo','Eventos','Bienestar','En familia','Todo incluido','Sol y Playa','Escapadas a ciudad','Montaña y Esquí','Lujo','Icónico','Acuático'],
    dests:{
...
function applyI18n(){
  ...
  if($('#destino')) $('#destino').placeholder = ph.dest;
  if($('#dates'))   $('#dates').placeholder   = ph.dates;
...
function setupNav(){
  ...
  if(langBtn){ langBtn.addEventListener('click',e=>{ e.preventDefault(); setLang(getLang()==='es'?'en':'es'); }); }
  $('#lang-current').textContent = getLang()==='es'?'ES':'GB';
  $('#cur-current').textContent  = curSymbol(getCur());
}

/* ---------- Destinations 2 niveles ---------- */
...
/* ---------- Date range (single input, 2 months) ---------- */
let startDate=null, endDate=null;
const fmtInput = d => !d ? '' : `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()}`;
const sameDay  = (a,b)=> a&&b&&a.getFullYear()===b.getFullYear()&&a.getMonth()===b.getMonth()&&a.getDate()===b.getDate();
const monthLabel= d => d.toLocaleString(getLang()==='es'?'es-ES':'en-GB',{month:'long',year:'numeric'});
const weekdayLabels=()=>I18N[getLang()].weekdays;

function renderCalendar(base){
  const panel=$('#date-range'), display=$('#dates');
  ...
  function monthHTML(first){
    ...
    const startGrid=add(firstDay,-((firstDay.getDay()+6)%7));
    let cells='';
    for(let i=0;i<42;i++){
    ...
    return `<div class="cal"><header><span class="m">${monthLabel(first)}</span></header>
      <div class="grid-days">${weekdayLabels().map(x=>`<div class="dow">${x}</div>`).join('')}${cells}</div></div>`;
  }
  ...
  $$('.day[data-date]').forEach(d=>{
    d.onclick=()=>{
      ...
      updateInput(); renderCalendar(startDate||base);
    };
  });
  $('#clearRange').onclick=()=>{ startDate=endDate=null; updateInput(); renderCalendar(base); };
  ...
  function updateInput(){
    ...
    const fmt=d=>d.toLocaleString(loc,{weekday:'short',day:'numeric',month:'short'});
    display.value = startDate ? (endDate ? `${fmt(startDate)} – ${fmt(endDate)}` : `${fmt(startDate)} – …`) : '';
  }
}
